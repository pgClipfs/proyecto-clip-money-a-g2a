...
export class IndexOperacionesComponent implements OnInit {
  ...
  buscarCuentaDestino() {
    if (this.form.alias != "" && this.form.cuenta != "") {
      console.log('entra');
      ...
      this.operacionesService.obtenerCuentaDestino(this.form.alias).subscribe((data: Cliente) => {
        if (data != null) {
          ...
          this.mensaje = "El CVU o Alias ingresado es incorrecto.";
          this.seccionBuscar = false;
          swal.fire('Ups', this.mensaje, 'warning');
        }
      })
    }
    else {
      this.mensaje = "No pueden quedar campos vacios.";
  ...
  transferir(): void {
    ...
    if (this.form.cuenta != undefined && this.form.monto != undefined && this.form.alias != undefined) {
      ...
      console.log(transferencia.alias);
      console.log(transferencia.id_cuenta_virtual);
